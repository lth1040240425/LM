...
using System.Text;
using LM.Core.Exceptions;
using LM.Core.Extensions;
using LM.Core.Properties;

namespace LM.Core.Filter
{
    /// <summary>
    /// 集合类型字符串排序操作类
    /// </summary>
    ...
    public static class CollectionPropertySorter<T>
    {
        ...
        private static LambdaExpression GetKeySelector(string keyName)
        {
            ...
            foreach (string propertyName in propertyNames)
            {
                ...
                if (property == null)
                {
                    throw new LMException(string.Format(Resources.ObjectExtensions_PropertyNameNotExistsInType, propertyName));
                }
